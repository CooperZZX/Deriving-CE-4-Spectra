%% 本程序用于实现从CE4-VD/CMOS的2BL头文件中提取出所有相关参数
%本程序运行一次约0.02s，远远快于extr_VDXML_par（0.2s）
% Author: Zhenxing Zhao
% Date: 2022.4.22
% Input parameter:
%   filename_VD2BL: The filename of CMOS 2BL. If the current filepath is
%   not the 2BL filepath, filename_SD2BL should be the full path.
% Output parameter:
%   Par_all: a table contains all the parameters in the VD2BL file
% Tested under MATLAB 2021a
function [Par_all]=extr_VD2BL_par(filename_VD2BL)
fID = fopen(filename_VD2BL);
...
Vector_Cartesian_3_Pointing=zeros(3,5);%XYZ
Angle_Pointing_Results=zeros(3,5);%incidence_angle，azimuth_angle，phase_angle
        ...
        while ~contains(line_x,'</Mission_Area>')
            ...
            if contains(line_x,'<Vector_Cartesian_3_Pointing>')
                while ~contains(line_x,'</Vector_Cartesian_3_Pointing>')
                    line_x=fgetl(fID);
                    %1
                    if contains(line_x,'<up_left_point_observe_vector>')
                        while ~contains(line_x,'</up_left_point_observe_vector>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</x>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                    ...
                    %2
                    if contains(line_x,'<down_left_point_observe_vector>')
                        while ~contains(line_x,'</down_left_point_observe_vector>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</x>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(1,2)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</y>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(2,2)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</z>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(3,2)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                    %3
                    if contains(line_x,'<up_right_point_observe_vector>')
                        while ~contains(line_x,'</up_right_point_observe_vector>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</x>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(1,3)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</y>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(2,3)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</z>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(3,3)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                    %4
                    if contains(line_x,'<down_right_point_observe_vector>')
                        while ~contains(line_x,'</down_right_point_observe_vector>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</x>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(1,4)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</y>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(2,4)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</z>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(3,4)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                    %5
                    if contains(line_x,'<center_point_observe_vector>')
                        while ~contains(line_x,'</center_point_observe_vector>')
                            ...
                            if contains(line_x,'</x>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(1,5)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</y>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(2,5)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</z>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Vector_Cartesian_3_Pointing(3,5)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                end
            ...
            if contains(line_x,'<Angle_Pointing_Results>')
                while ~contains(line_x,'</Angle_Pointing_Results>')
                    ...
                    %2
                    if contains(line_x,'<up_left_point>')
                        while ~contains(line_x,'</up_left_point>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</incidence_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(1,2)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</azimuth_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(2,2)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</phase_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(3,2)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                    %3
                    if contains(line_x,'<down_left_point>')
                        while ~contains(line_x,'</down_left_point>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</incidence_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(1,3)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</azimuth_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(2,3)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</phase_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(3,3)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                    %4
                    if contains(line_x,'<up_right_point>')
                        while ~contains(line_x,'</up_right_point>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</incidence_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(1,4)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</azimuth_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(2,4)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</phase_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(3,4)=str2double(line_match(2:end-1));
                            end
                        end
                    end
                    %5
                    if contains(line_x,'<down_right_point>')
                        while ~contains(line_x,'</down_right_point>')
                            line_x=fgetl(fID);
                            if contains(line_x,'</incidence_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(1,5)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</azimuth_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(2,5)=str2double(line_match(2:end-1));
                            end
                            if contains(line_x,'</phase_angle>')
                                line_match=char(regexp(line_x,'>[^<]*<','match'));
                                Angle_Pointing_Results(3,5)=str2double(line_match(2:end-1));
                            end
                        end 
                    end
...
Par_all.Properties.VariableNames={'sequence_id','Time_start_stop','Lon_Lati/deg','Rover_Loc','Lander_Loc','Angle/deg','Angl_C','Angl_UL','Angl_DL','Angl_UR','Angl_DR','solar_ang',...
                                      'XYZ/m','Rover_XYZ','Center_XYZ','Vect_UL','Vect_DL','Vect_UR','Vect_DR','Vect_C'};
fclose(fID);
end